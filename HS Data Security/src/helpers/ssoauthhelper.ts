...
import { paneToDisplay } from "./AddinPanelType";
let retryGetAccessToken = 0;

export async function getGraphData(paneType: string): Promise<void> {
  try {
    ...
    if (exchangeResponse.error) {
      ...
      handleAADErrors(paneType, exchangeResponse);
    } else {
      ...
      //paneToDisplay(paneType, response);
      paneToDisplay(paneType, response);
...
function handleAADErrors(paneType: string, exchangeResponse: any): void {
